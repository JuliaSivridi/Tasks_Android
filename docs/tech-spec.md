...
**Displayed in `TasksTopAppBar` as a sync icon button (three visual states):**
- `Idle` → `CloudDone` icon (tinted `onSurfaceVariant`)
- `Pending(count)` → `CloudUpload` icon with a muted counter badge; badge uses `Color.Transparent` container + `onSurfaceVariant` text (same color as the icon, not the default red)
- `Syncing` → `Sync` icon with infinite rotation animation (1 s, `LinearEasing`)
...
1. Tasks with a valid `deadlineDate` and calendar events are mixed together into `combinedDated`, sorted by:
   - Priority weight ascending: URGENT=0, IMPORTANT=1, NORMAL=2; calendar events count as NORMAL (2) so they interleave with same-priority tasks
...
2. Tasks WITHOUT a deadline (`deadlineDate.isBlank()`) are appended **after** all dated items, sorted by priority weight only
...
**Layout (mirrors `TaskItem`):**
```
Row(start=8dp, end=4dp, top=4dp, bottom=4dp) {
    Box(40dp) { CalendarMonth icon 24dp, tinted calColor }
    Spacer(8dp)
    Column(weight=1f) {
        Text(title, maxLines=2)
        FlowRow { [recurring icon?] [time label?] · [CalendarMonth 14dp] [calendar name] }
    }
    Spacer(4dp)
    Box(40dp, clickable) { Schedule icon 18dp }   // only when onEdit/onDelete non-null
    Box(40dp, clickable) { MoreHoriz icon 18dp }  // only when onEdit/onDelete non-null
}
```
- `CalendarMonth` icon (24dp) tinted with `event.calendarColor` — identifies the calendar
- Title: clickable when `onEdit` is non-null; 2-line max with ellipsis
- **Recurring icon** (`Icons.Outlined.Autorenew`, 14dp, muted): shown when `event.recurringEventId.isNotBlank()`; grouped with `timeLabel` in a `Row(defaultMinSize(minHeight=20dp), CenterVertically)` for consistent vertical alignment
- `timeLabel`: for timed events — `"HH:MM — HH:MM"` or `"HH:MM"`; for all-day + `showDate=true` — `"d MMM"` / `"Today"` / `"Tomorrow"`; for all-day + `showDate=false` — empty; colored with `deadlineColor(deadlineStatus(startDate))`
- Calendar name row: `CalendarMonth` 14dp (tinted) + calendar name text — wraps naturally, no ellipsis
...
| showExpandSlot | false | Reserve a 32dp slot even for leaf tasks (FolderScreen passes `true` so all rows align) |
...
**Row 1:**
```
[expand 32dp?] [checkbox 40dp] [title weight=1] [schedule btn 40dp] [more btn 40dp]
```
- **Expand slot**: 32dp `Box`. Shown when `showExpandSlot=true` OR `hasChildren=true`. When `hasChildren=true` the box is `clickable` and shows a `ChevronRight` / `ExpandMore` icon (24dp); otherwise the box is empty (alignment placeholder). No spacer between expand box and checkbox.
...
- Action buttons: `Box(Modifier.size(40.dp).clickable {...}) { Icon(18dp) }` — tighter than `IconButton` (48dp min) to reduce visual spacing between icons
- When completed: Restore + Delete buttons instead of schedule/more

**Row 2 (conditional, when any metadata exists) — `FlowRow`:**
```
[recurring icon + deadline label grouped] [label chips] [folder] [subtask count]
```
- Indented by `depth × 20 + (expand slot width) dp`
- Recurring icon + deadline label are wrapped together in a `Row(defaultMinSize(minHeight=20dp), CenterVertically, spacedBy(4dp))` so the 14dp icon stays vertically centered regardless of whether a deadline label is also present
- Each metadata element uses `FlowRow(horizontalArrangement=spacedBy(6dp), verticalArrangement=spacedBy(2dp))` — wraps to next line when it overflows, no truncation
